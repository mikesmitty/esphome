...
bool WytClimate::query_state_(bool read_only) {
  ...
  if (memcmp(this->raw_state_, buf_state_, WYT_QUERY_RESPONSE_SIZE) != 0) {
    state_changed = true;
    memcpy(this->raw_state_, buf_state_, WYT_QUERY_RESPONSE_SIZE);
  }

  if (state_changed) {
    char hex_buf[WYT_QUERY_RESPONSE_SIZE * 3];
    ESP_LOGV(TAG, "Response: %s", format_hex_pretty_to(hex_buf, buf_state_, WYT_QUERY_RESPONSE_SIZE));
  }
...
void WytClimate::update_sensors_() {
  if (this->defrost_binary_sensor_ != nullptr && this->defrosting_ != this->is_defrosting()) {
  ...
  if (this->power_sensor_ != nullptr && this->power_usage_ != this->get_power_usage()) {
    ...
    this->power_sensor_->publish_state(this->power_usage_);
  }
}
...
SetCommand WytClimate::command_from_response(const StateResponse &response) {
  ...
  command.gen_mode = 0x0;  // Always disable gen_mode for now
...
void WytClimate::send_command(SetCommand &command) {
  ...
  char hex_buf[WYT_STATE_COMMAND_SIZE * 3];
  ESP_LOGI(TAG, "Send: %s", format_hex_pretty_to(hex_buf, command.bytes, WYT_STATE_COMMAND_SIZE));
...
optional<std::string> WytClimate::get_pioneer_custom_fan_mode() {
...
void WytClimate::do_remote_temp(float temp_c, bool beeper) {
  ...
  remote_base::PioneerWytData fan_data(fan_command.bytes, WYT_REMOTE_COMMAND_SIZE);
  remote_base::PioneerWytData general_data(general_command.bytes, WYT_REMOTE_COMMAND_SIZE);
